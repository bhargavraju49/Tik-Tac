  ...
  useEffect(() => {
    setTimeout(() => {
      ...
      for (let i = 0; i < k.length; i++) {
        ...
        if (
          (x1[k[i][0]] == z2 && x1[k[i][1]] == z2) ||
          (x1[k[i][0]] == z2 && x1[k[i][2]] == z2) ||
          (x1[k[i][2]] == z2 && x1[k[i][1]] == z2)
        ) {
          for (let j = 0; j < 3; j++) {
            if (x1[k[i][j]] != z1) {
              if (x1[k[i][j]] != z2) {
                index = k[i][j];
                console.log(index, "ifff");
              }
            }
          }
          break;
        } else if (
          (x1[k[i][0]] == z1 && x1[k[i][1]] == z1) ||
          (x1[k[i][0]] == z1 && x1[k[i][2]] == z1) ||
          (x1[k[i][2]] == z1 && x1[k[i][1]] == z1)
        ) {
          for (let j = 0; j < 3; j++) {
            if (x1[k[i][j]] != z2) {
              if (x1[k[i][j]] != z1) {
                index = k[i][j];
                console.log(index, "eellllifff");
              }
            }
          }
          break;
      ...
      console.log(index, "___________________________%%%%");
